...
#else
        /* No lv_drivers display backend selected.
         * Use a safe default to keep build working. */
        LvDisplay(drv, 480, 320)
#endif
{
}
...
#if !LV_DISP_BUFFER_STATIC
	buf1_1.reset((lv_color_t*)lv_mem_alloc(sizeof(lv_color_t) * w * 100));
	buf1_2.reset((lv_color_t*)lv_mem_alloc(sizeof(lv_color_t) * w * 100));
	lv_disp_draw_buf_init(disp_buf1.get(), buf1_1.get(), buf1_2.get(), w * 100);
#else
	/* Static buffer should not depend on MONITOR_* macros (they might be disabled).
	 * Use a conservative fixed width. */
	static lv_color_t buf1_1[480 * 100];
	static lv_color_t buf1_2[480 * 100];
	lv_disp_draw_buf_init(disp_buf1.get(), buf1_1, buf1_2, 480 * 100);
#endif
