...
/* Runtime resolution override (default: MONITOR_HOR_RES/MONITOR_VER_RES) */
static int32_t g_mon_hor = MONITOR_HOR_RES;
static int32_t g_mon_ver = MONITOR_VER_RES;
#endif
...
{
    if(hor_res > 0) g_mon_hor = hor_res;
    if(ver_res > 0) g_mon_ver = ver_res;
}
#endif
...
//    printf("x1:%d,y1:%d,x2:%d,y2:%d\n", area->x1, area->y1, area->x2, area->y2);
    ...
    for(y = area->y1; y <= area->y2 && y < disp_drv->ver_res; y++) {
#if MONITOR_RUNTIME_RESOLUTION
        memcpy(&monitor.tft_fb[y * (uint32_t)g_mon_hor + area->x1], color_p, w * sizeof(lv_color_t));
#else
...
static void sdl_event_handler(lv_timer_t * t)
{
    ...
    while(SDL_PollEvent(&event)) {
        /* Fallback quit handling: some environments don't reliably trigger SDL event filters */
...
#if MONITOR_RUNTIME_RESOLUTION
    SDL_SetWindowPosition(monitor.window, x + (g_mon_hor * MONITOR_ZOOM) / 2 + 10, y);
    SDL_SetWindowPosition(monitor2.window, x - (g_mon_hor * MONITOR_ZOOM) / 2 - 10, y);
#else
...
#if MONITOR_RUNTIME_RESOLUTION
                              g_mon_hor * MONITOR_ZOOM, g_mon_ver * MONITOR_ZOOM, 0);       /*last param. SDL_WINDOW_BORDERLESS to hide borders*/
#else
...
#else
#if MONITOR_RUNTIME_RESOLUTION
    m->tft_fb = (uint32_t *)malloc(sizeof(uint32_t) * (uint32_t)g_mon_hor * (uint32_t)g_mon_ver);
    memset(m->tft_fb, 0x44, (uint32_t)g_mon_hor * (uint32_t)g_mon_ver * sizeof(uint32_t));
#else
...
static void window_update(monitor_t * m)
{
...
#if MONITOR_RUNTIME_RESOLUTION
    SDL_UpdateTexture(m->texture, NULL, m->tft_fb, (int)g_mon_hor * (int)sizeof(uint32_t));
#else
...
#if MONITOR_RUNTIME_RESOLUTION
    SDL_UpdateTexture(m->texture, NULL, m->tft_fb_act, (int)g_mon_hor * (int)sizeof(uint32_t));
#else
    SDL_UpdateTexture(m->texture, NULL, m->tft_fb_act, MONITOR_HOR_RES * sizeof(uint32_t));
#endif
#endif
    ...
    r.x = 0; r.y = 0;
#if MONITOR_RUNTIME_RESOLUTION
    r.w = g_mon_hor; r.w = g_mon_ver;
#else
    r.w = MONITOR_HOR_RES; r.w = MONITOR_VER_RES;
#endif
